...
cpdef sprite_sheet_per_pixel(str file_, int chunk_, int rows_, int columns_):
    """
    ...
    * All sprites must be uniform (equivalent width and height)
    ...
    :param chunk_: integer; Size in pixels of a single sprite (ex 200 pixels for a sprite size 200x200).  
    :param rows_: integer; number of rows 
    :param columns_: integer; number of columns 
    :return: Returns a python list containing all the sprites converted to fast blit pygame.convert_alpha()
    """
    ...
    try:
        ...
        raise ValueError("Cannot load spritesheet %s.\n"
                         ...
                         "This version is compatible with 32-bit spritesheet with layer alpha" % (file_, e))
    cdef:
...
cpdef sprite_sheet(str file_, int chunk_, int rows_, int columns_):
    ...
    * All sprites must be uniform (equivalent width and height)
    ...
    :param rows_: integer; number of rows 
    :param columns_: integer; number of columns 
    :return: Returns a python list containing all the sprites converted to fast blit pygame.convert()
    """
...
cpdef Sprite_Sheet_Uniform_RGB(
    ...
    * If the sprite sheet is an RGBA image with per pixel information, the final sprites will be trimmed of 
    ...
    * This method will raise an exception for 32 or 8 bit surface (sprite sheet) 
    ...
    :param file_: string; full path to the SpriteSheet such as "C:\\directory\\file"
    :param size_: integer; Size in pixels of a single sprite (ex 200 pixels for a sprite size 200x200).  
    :param rows_: integer; number of rows 
    :param columns_: integer; number of columns 
    :return: Returns a python list containing all the sprites converted for fast blit (pygame.convert())
    """
    if not pygame.display.get_init():
        raise ValueError("Display module has not been initialized")
    ...
    try:
        ...
        raise ValueError("Cannot load spritesheet %s.\n "
                         "Invalid file format or bitsize incompatible\nError %s\n"
                         "This version is compatible with 24-bit spritesheet without layer alpha only" % (file_, e))
...
# TODO DOC surface_split and explain what it does
...
cpdef Sprite_Sheet_Uniform_RGBA(
    ...
    * This method will raise an exception for 24 or 8 bit surface (sprite sheet) 
    ...
    :param rows_: integer; number of rows 
    :param columns_: integer; number of columns 
    :return: Returns a python list containing all the sprites converted for fast blit (pygame.convert_alpha())
    """
    cdef int bitsize
    ...
    if os.path.isfile(path_to_surface):
        ...
        try:
            ...
            raise ValueError("Spritesheet is not a valid surface or not supported by Pygame\nError %s" %e)
    ...
    try:
        ...
        raise ValueError("Spritesheet cannot be converted into a 3d numpy array shape (h, w, 4) RGBA"
...
cpdef sprite_sheet_fs8(str file_, int chunk_, int columns_,
    ...
    * Return sprite animation and sprite transparency is set with colorkey (black color is full transparency)
    ...
    * set color_ to None if you do not wish to set a transparency colorkey
    ...
    :return: list, Return textures (surface) containing colorkey transparency (black color is full transparency)
...
cpdef sprite_sheet_fs8_alpha(str file_, int chunk_, int columns_,
